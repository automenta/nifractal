...
public class NiftyEventAnnotationProcessor {
  ...
  public static void process(final String name, final Object obj) {
    processOrUnprocess(name, obj, true);
  }

  public static void unprocess(final String name, final Object obj) {
    processOrUnprocess(name, obj, false);
  }

  private static void processOrUnprocess(final String name, final Object obj, final boolean add) {
    if (obj == null) {
      return;
    }
    ...
    for (Method method : methods) {
      ...
      if (niftyEventSubscriber != null) {
        ...
        process(name, niftyEventSubscriber, obj, method, add);
      }
    }
  }

  private static void process(String name, final NiftyEventSubscriber annotation, final Object obj, final Method method, final boolean add) {
    String id = annotation.id();
    String pattern = annotation.pattern();
    ensureNotNull(id, pattern);
    ensureMethodParamCount(method.getParameterTypes());
    EventService eventService = getEventService(name);
  ...
  public static EventService getEventService(String name) {
    return EventServiceLocator.getEventService(name);
  }
